...
const TYPE_CONFIG = {
  ...
  gemini: { color: "bg-amber-600", text: "text-white" },
  deepseek: { color: "bg-zinc-800", text: "text-white" },
  qwen: { color: "bg-emerald-700", text: "text-white" },
  doubao: { color: "bg-stone-700", text: "text-white" },
  custom: { color: "bg-zinc-100", text: "text-zinc-600" },
};

const COPY = {
  "zh-CN": {
    eyebrow: "Intelligence Configuration",
    title: "AI 服务商管理",
    subtitle: "查看服务端 AI 状态，并维护本地服务商配置草稿。",
    add: "添加渠道",
    save: "保存所有配置",
    saved: "已保存",
    serverTitle: "服务端 AI 配置状态",
    serverDesc:
      "当前编辑器的 AI 功能读取服务端环境变量。本页下方配置保存在浏览器本地，可作为配置草稿使用，不会影响已上线的服务端调用。",
    modelLoading: "模型读取中",
    keyReady: "API Key 已配置",
    keyMissing: "API Key 未配置",
    serverLoading: "读取服务端配置",
    active: "Active",
    noModel: "未配置模型",
    activate: "点击激活",
    activated: "已激活",
    ...
    baseUrlLabel: "Base URL (API 地址)",
    keyLabel: "API Key",
    customName: "自定义渠道",
    keepOne: "请至少保留一个 AI 渠道。",
    footer: "QingJiao Intelligence Module v2.0",
  },
  "en-US": {
    eyebrow: "Intelligence Configuration",
    title: "AI Providers",
    subtitle: "Review server AI status and maintain local provider drafts.",
    add: "Add provider",
    save: "Save settings",
    saved: "Saved",
    serverTitle: "Server AI status",
    serverDesc:
      "Editor AI features use server environment variables. Local provider settings below are drafts stored in the browser and do not affect deployed server calls.",
    modelLoading: "Loading model",
    keyReady: "API Key configured",
    keyMissing: "API Key missing",
    serverLoading: "Reading server config",
    active: "Active",
    ...
    modelLabel: "Model ID",
    modelPlaceholder: "Example: gpt-4o",
    baseUrlLabel: "Base URL",
    keyLabel: "API Key",
    customName: "Custom provider",
    keepOne: "Keep at least one AI provider.",
    footer: "QingJiao Intelligence Module v2.0",
  },
...
export default function AIPage() {
  ...
  useEffect(() => {
    ...
    // 使用异步任务确保初始化状态更新与 Hydration 流程解耦
        ...
        <div className="mb-10 p-8 bg-zinc-900 rounded-[2.5rem] text-white overflow-hidden relative group shadow-xl shadow-zinc-200">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex items-start gap-4">
              ...
              <div className="space-y-1">
                <h3 className="text-xl font-bold">{copy.serverTitle}</h3>
                ...
                <div className="flex flex-wrap gap-2 pt-3 text-[10px] font-bold uppercase tracking-widest">
                  <span className="rounded-full bg-white/10 px-3 py-1 text-zinc-300">
                    {serverStatus?.model || copy.modelLoading}
                  </span>
                  <span
                    className={cn(
                  ...
                  >
                    {serverStatus?.hasApiKey ? copy.keyReady : copy.keyMissing}
                  </span>
                </div>
              </div>
            </div>
            <div
              ...
              title={serverStatus?.baseUrl || copy.serverLoading}
            >
              {serverStatus?.baseUrl || copy.serverLoading}{" "}
        ...
        <div className="space-y-4">
          <AnimatePresence>
            {providers.map((p) => {
              ...
              const config =
                TYPE_CONFIG[p.type as keyof typeof TYPE_CONFIG] ||
                TYPE_CONFIG.custom;
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="space-y-0.5">
                        ...
                        <p className="text-[10px] text-zinc-400 font-mono tracking-widest uppercase italic">
                          {p.model || copy.noModel}
                        </p>
                      </div>
                    </div>
                        ...
                        <div className="w-full lg:w-40 shrink-0 flex flex-col gap-3">
                          ...
                          >
                            {p.isActive ? (
                              ...
                              <div className="w-3.5 h-3.5 border border-zinc-300 rounded-full" />
                            )}
                            {copy.setActive}
                          </button>
