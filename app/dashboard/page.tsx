...
const resumes = [
  ...
  { id: "2", title: "前端开发工程师-廖欢全", lastModified: "1 周前", theme: "#3b82f6" },
];
    ...
    <div className="flex-1 min-h-full flex flex-col p-8 lg:p-12 animate-in fade-in zoom-in-95 duration-700 ease-out fill-mode-forwards">
      ...
      <div className="group flex items-center justify-between p-4 bg-emerald-50/80 backdrop-blur-sm border border-emerald-200 rounded-2xl shadow-sm hover:shadow-md hover:bg-emerald-50 transition-all duration-300 mb-10 overflow-hidden relative">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-emerald-900 leading-tight">数据已成功备份</p>
            <p className="text-xs text-emerald-600/80">所有的简历配置已同步至浏览器本地存储，随时随地开启编辑。</p>
          </div>
        </div>
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 pb-8 border-b border-zinc-200/50">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-zinc-900 mb-2">我的简历</h1>
          <p className="text-zinc-500 text-sm">通过极简的 SaaS 控制面板管理您的所有简历文档。</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        ...
        >
          ...
          <div className="mt-5 text-center px-6">
            ...
            <p className="text-xs text-zinc-500 leading-relaxed font-medium">创建一个全新的简历配置以开始您的求职之旅</p>
          </div>
        </Link>

        {/* B. 已存在简历卡列表 */}
        {resumes.map((resume) => (
          <div key={resume.id} className="group relative bg-white border border-zinc-200 rounded-[2rem] overflow-hidden shadow-sm hover:shadow-2xl hover:shadow-zinc-200 transition-all duration-500 hover:-translate-y-2 flex flex-col h-[320px]">
             {/* 缩略图区 - A4 比例预览容器 */}
             ...
             <div className="p-5 flex items-center justify-between">
                <div>
                   <h3 className="font-bold text-zinc-900 text-base mb-1 line-clamp-1">{resume.title}</h3>
                   <div className="flex items-center gap-2">
                      <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: resume.theme }} />
                      <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">{resume.lastModified}修改</span>
                   </div>
                </div>
                <div className="flex items-center gap-1">
                   <Link href="/editor" className="p-2.5 bg-zinc-50 rounded-xl text-zinc-600 hover:bg-zinc-900 hover:text-white transition-all duration-300">
                      <FileEdit size={16} />
                   </Link>
